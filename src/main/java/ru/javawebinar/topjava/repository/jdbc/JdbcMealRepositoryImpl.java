...
public class JdbcMealRepositoryImpl implements MealRepository {
    ...
    public Meal save(Meal meal, int userId) {
        ...
        if(meal.isNew()) {
            ...
            namedParameterJdbcTemplate.update("UPDATE user_meals SET datetime=:datetime, description=:description, " +
    ...
    public List<Meal> getAll(int userId) {
        return jdbcTemplate.query("SELECT * FROM user_meals WHERE user_id=?", rowMapper, userId);
    }

    @Override
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return jdbcTemplate.query("SELECT * FROM user_meals WHERE user_id=? AND datetime BETWEEN ? AND ?", rowMapper, userId, startDate, endDate);
    }
}
